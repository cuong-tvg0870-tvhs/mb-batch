...
9. **Advantage+ Audience Age Controls & Suggestions**: Khi bật Advantage+ Audience, UI bản nháp tách giống Meta Ads Manager: phần Controls có `Minimum age` gửi sang Meta bằng `audience_controls.min_age` ở cấp ad set, còn phần gợi ý đối tượng vẫn gửi trong `targeting.age_min`/`targeting.age_max`. Khi user đổi `Minimum age`, UI tự kéo `age_min` gợi ý về cùng số để tránh Ads Manager hiển thị lệch; nếu user chọn `age_min` cao hơn `Minimum age`, UI/preview giải thích rằng Meta sẽ hiển thị minimum age hiệu lực là số lớn hơn. `minimum_age` và `use_age_min_control` chỉ là metadata tương thích draft/UI nội bộ và bị strip khỏi `targeting` trước khi publish; special ad categories khóa 18–65 theo policy.
...
14. **Contribution Status Transitions & History Log**: Cho phép người tạo ý kiến đóng góp chủ động Đóng (`CLOSED`) hoặc Yêu cầu xử lý lại (`REOPENED` - "Chưa đạt yêu cầu") thông qua API `PATCH /contributions/:id/status`. Khi cập nhật kèm lý do, hệ thống tự động chèn tin nhắn thông báo thay đổi trạng thái vào danh sách thảo luận (`userFeedback.messages`) để ghi lại lịch sử xử lý. Trên UI, ngày dự kiến (`expectedProcessedAt`) và ngày xử lý (`processedAt`) được hiển thị tại cột "Thời gian xử lý", đi kèm bộ lọc nhanh "Đóng góp của tôi" sử dụng URL Search Params để đồng bộ trạng thái tìm kiếm.
