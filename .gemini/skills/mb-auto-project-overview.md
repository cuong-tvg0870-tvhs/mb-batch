...
Auto Launch email notification is opt-in per rule through `notifyOnLaunch` and `notifyOnError` and always resolves the current email of the rule owner (`createdById`). `AutoLaunchNotifyService` in `mb-ads` polls only completed scheduled runs, atomically claims `AutoLaunchRuleRun.notifiedAt`, and sends the branded `auto-launch-notify` template; manual `run-now` rows (`dedupeKey = null`) and routine skips never send mail. `completedAt` is the finalization signal because a newly claimed run temporarily uses `FAILED` while Meta publishing may still be in progress. A `LAUNCHED` run requested as `PUBLISH_IMMEDIATELY` with no `metaId` is reported as a publish warning, not a clean success. The canonical additive migration is `mb-db/prisma/migrations/20260721170000_add_auto_launch_email_notify`.
