...
12. **Folder-based Content Permission Checking for CIDs**: Các bản ghi LarkRecord (CIDs) chỉ hiển thị với người dùng không phải admin khi họ được phân quyền truy cập vào cấu trúc thư mục tương ứng. Hệ thống xác định các thư mục được phép (qua project mà họ là MANAGER/LEADER, hoặc qua các bản ghi FolderMember trực tiếp) và mở rộng đệ quy ra toàn bộ các thư mục con trong bộ nhớ. Sau đó, truy vấn được lọc trực tiếp dưới DB bằng mệnh đề `OR` khớp với hierarchy (`project_name`, `brand_name`, `product_code`) hoặc `creative_asset.folderId` để bảo toàn phân trang mà không gây ra lỗi N+1 query.
